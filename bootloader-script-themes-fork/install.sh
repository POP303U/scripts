...
function select_theme() {
    ...
    timeouts=('skip-timeout' 'wait-indefinitely' '3' '5' '10' '20' '30' '60')

    PS3=$(echo_prompt '\nChoose The Timeout You Want: ')
    select TIMEOUT_SELECTION in "${timeouts[@]}"; do
        case "${TIMEOUT_SELECTION}" in
            'skip-timeout')
                splash 'Skipping grub timeout'
                TIMEOUT_SELECTION=0
                break;;
            'wait-indefinitely')
                splash 'Waiting for selection in grub'
                TIMEOUT_SELECTION=-1
                break;;
            '3')
                splash 'Setting timeout to 3 Seconds'
                break;;
            '5')
                splash 'Setting timeout to 5 Seconds'
                break;;
            '10')
                splash 'Setting timeout to 10 Seconds'
                break;;
            '20')
                splash 'Setting timeout to 20 Seconds'
                break;;
            '30')
                echo_info 'Setting timeout to 30 Seconds'
                break;;
            '60')
...
function config_grub() {
    ...
    echo_primary "Setting grub timeout to $TIMEOUT_SELECTION seconds"
    # remove default timeout if any
    echo_info "sed -i '/GRUB_TIMEOUT=/d' /etc/default/grub"
    sed -i '/GRUB_TIMEOUT=/d' /etc/default/grub

    echo_info "echo 'GRUB_TIMEOUT=\"$TIMEOUT_SELECTION\"' >> /etc/default/grub"
    echo "GRUB_TIMEOUT="$TIMEOUT_SELECTION"" >> /etc/default/grub
